...
$features = filter_var($features, FILTER_SANITIZE_STRING);
   ...
   $update_product = $conn->prepare("UPDATE `products` SET name = ?, price = ?, details = ?, features = ? WHERE id = ?");
   $update_product->execute([$name, $price, $details, $features, $pid]);
   ...
   if(!empty($image_01)){
      if($image_size_01 > 20000000000000000){
   ...
   if(!empty($image_03)){
      ...
      if(!empty($image_04)){
         if($image_size_04 > 2000000){
            ...
            $update_image_04 = $conn->prepare("UPDATE `products` SET image_04 = ? WHERE id = ?");
            $update_image_04->execute([$image_04, $pid]);
            move_uploaded_file($image_tmp_name_04, $image_folder_04);
            unlink('../uploaded_img/'.$old_image_04);
      ...
      if(!empty($image_05)){
         if($image_size_05 > 2000000){
            ...
            $update_image_05 = $conn->prepare("UPDATE `products` SET image_05 = ? WHERE id = ?");
            $update_image_05->execute([$image_05, $pid]);
            move_uploaded_file($image_tmp_name_05, $image_folder_05);
            unlink('../uploaded_img/'.$old_image_05);
...
<section class="update-product">
   ...
   <form action="" method="post" enctype="multipart/form-data">
    ...
    <input type="text" name="name" required class="box" maxlength="10000" placeholder="enter product name" value="<?= $fetch_products['name']; ?>">
    <span>update price</span>
    <input type="number" name="price" required class="box" min="0" max="9999999999" placeholder="enter product price" onkeypress="if(this.value.length == 10) return false;" value="<?= $fetch_products['price']; ?>">
    <span>Features</span>
<textarea name="features" placeholder="enter product features" class="box" required maxlength="500000000" cols="30" rows="10"><?= $fetch_products['features']; ?></textarea>

    <span>update details</span>
    <textarea name="details" class="box" required cols="30000000" rows="1000000"><?= $fetch_products['details']; ?></textarea>
